...
def convert_visuals_pb(vis, urdfLinks, urdfJoints, p0):
    b2vis={}
    uid = -1
...
#          v.geom_meshscale=(0.0001,0.0001,0.0001)
...
#                                  )
#                              ))
#          vis[vis_name].set_object(g.Mesh(g.ObjMeshGeometry.from_file(v.geom_meshfilename)))
          vis[vis_name].set_object(g.StlMeshGeometry.from_file(v.geom_meshfilename), g.MeshLambertMaterial(color=0xFFFFFF))
#                          g.MeshLambertMaterial(
#                              map=g.ImageTexture(
#                                  image=g.PngImage.from_file(texture_path)
#                                  )
#          vis[vis_name].set_transform(tf.scale_matrix(0.000001))
...
#      mat3 = mat3 * 0.001
      ...
      mat_array = np.vstack([mat_array[:3,:]*0.001, [0,0,0,1]])
      vis[v.vis_name].set_transform(mat_array)
#      vis[v.vis_name].set_transform(tf.scale_matrix(0.000001))
